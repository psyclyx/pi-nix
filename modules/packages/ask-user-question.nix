...
let
  ...
  inherit (lib) types;
  cfg = config.pi.packages.askUserQuestion;
  entry = packageLib.registryEntry registry "ask-user-question";
  ...
  source = packageLib.npmPackageSource {
    ...
    patches = ''
      substituteInPlace "$out/config.ts" \
        --replace-fail 'const CONFIG_PATH = configPath("rpiv-ask-user-question");' \
        'const CONFIG_PATH = process.env.RPIV_ASK_USER_QUESTION_CONFIG_PATH?.trim() || configPath("rpiv-ask-user-question");'
    '';
  };
  askUserQuestionConfig = packageLib.clean {
    guidance = packageLib.clean cfg.guidance;
  };
in
{
  options.pi.packages.askUserQuestion =
    packageLib.packageResourceOptions {
      defaultSource = source;
      description = "RPiV structured user-question tool for Pi";
    }
    // {
      guidance = {
  ...
  config = lib.mkIf cfg.enable {
    ...
    pi.files."config/rpiv-ask-user-question/config.json" = askUserQuestionConfig;
    pi.agentDirEnvironment.RPIV_ASK_USER_QUESTION_CONFIG_PATH = "config/rpiv-ask-user-question/config.json";
  };
}
